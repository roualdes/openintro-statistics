...
resample <- function(x) {
    ...
    xbar <- vector("list", R)
    for (i in seq_len(R)) {
        xbar[[i]] <- mean(pa[sample(nr, size=x, replace=TRUE)])
    }
    unlist(xbar)
}

df <- tidyr::gather(tibble::data_frame("n=5"=resample(5),
                         "n=30"=resample(30),
                         "n=100"=resample(100)),
                   ss, xbar)
@
...
ggplot2::ggplot(data=pennies, ggplot2::aes(x=age)) +
    ggplot2::geom_histogram(bins=10) +
        ggplot2::labs(x="Pennies ages (years)")
@
...
% TODO pick up here for replacing plots; 2016-12-21
