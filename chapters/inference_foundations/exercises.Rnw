...
\eoce{\qt{Identify the parameter, Part II\label{identify_parameter_2}}
...
\eoce{\qt{College credits\label{college_credits}}
  ...
  A college counselor is interested in estimating how many credits a
student typically enrolls in each semester. The counselor decides to
randomly sample 100 students by using the registrar's database of
students. The histogram below shows the distribution of the number of
credits taken by these students. Sample statistics for this
distribution are also provided.

  \begin{minipage}[c]{0.75\textwidth}
      <<collegeCreditsHist, opts.label="plot">>=
  ggplot2::ggplot(data=openintro::credits, ggplot2::aes(x=credits)) +
...
\begin{minipage}[c]{0.75\textwidth}
    <<adultHeightsHist, opts.label="plot">>=
  ggplot2::ggplot(data=openintro::bdims, ggplot2::aes(x=hgt)) +
      ggplot2::geom_histogram(bins=10) +
...
\item Is a person who is 1m 80cm (180 cm) tall considered unusually
  tall? And is a person who is 1m 55cm (155cm) considered unusually
  short? Explain your reasoning.
...
\eoce{\qt{Hen eggs\label{hen_eggs}}

  The distribution of the number of eggs laid by a certain species of
  hen during their breeding period has a mean of 35 eggs with a
  standard deviation of 18.2. Suppose a group of researchers randomly
  samples 45 hens of this species, counts the number of eggs laid
...
<<thanksgivingSpendingIntroPopHist, opts.label="plot">>=
ggplot2::ggplot(data=openintro::tgSpending, ggplot2::aes(x=spending)) +
    ggplot2::geom_histogram(bins=10) +
...
<<exlusiveRelationshipsRelHist, opts.label="plot">>=
survey <- suppressMessages(readr::read_csv("../../data/survey.csv"))
ggplot2::ggplot(data=na.omit(survey), ggplot2::aes(x=excl_relation)) +
    ggplot2::geom_histogram(bins=10) +
...
<<ageFirstMarriage, opts.label="plot">>=
ggplot2::ggplot(data=openintro::ageAtMar, ggplot2::aes(x=age)) +
    ggplot2::geom_histogram(bins=10) +
