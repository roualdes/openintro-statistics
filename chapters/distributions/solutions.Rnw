...
\eocesol{
  ...
  (a)~No. The cards are not independent. For example, if the first card is an
...
\eocesol{
  ...
  (a)~$\Sexpr{1-pbl_3.6.23}^2\times \Sexpr{pbl_3.6.23} = \Sexpr{pa_3.6.23}$. \\
...
\eocesol{
  ...
  (a)~$\mu = \Sexpr{m_3.6.27}$, $\sigma = \Sexpr{sd_3.6.27}$
  (b)~$Z = \Sexpr{z_3.6.27}$. Since the z-score is \Sexpr{not} greater
  than $3$ standard deviations away from the mean we are \Sexpr{not}
  surprised.
...
\eocesol{
  ...
  Want to find the probability that there will be $\Sexpr{x_3.6.29}$
